...
async function openDetail(id) {
  ...
  // Vérification côté client : le copropriétaire ne peut voir que ses tickets + critiques
  if (!canViewTicket(t)) {
  ...
  // Le syndic peut changer le statut mais pas les photos
  const showPhotoUpload = (isManager() || isAuthor) && !isSyndic();
  ...
  d($('m-detail-body'), `
    ...
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:18px;padding:14px;background:var(--bg);border-radius:var(--r);border:1px solid var(--border);">
      ...
      ${isSyndic() ? `<span style="font-size:11px;color:var(--text-3);">Mode syndic — vous pouvez mettre à jour le statut et commenter.</span>` : ''}
    ...
    <div style="margin-top:4px;">
      ...
      <div style="position:relative;">
        <textarea id="new-comment" class="textarea" style="min-height:66px;"
          placeholder="${isSyndic() ? 'Ajouter un commentaire visible par les résidents…' : 'Ajouter un commentaire... (tapez @ pour mentionner quelqu\'un)'}"
...
async function uploadTicketPhoto(ticketId, input) {
  if (isSyndic()) return;  // Le syndic ne peut pas uploader de photos
...
async function exportTicketPDF(ticketId) {
  ...
  <div class="header">
    ...
    <div class="header-right">
      <div class="ref" style="font-size:13px;font-weight:700;">N° ${t.id.substring(0,8).toUpperCase()}</div>
  ...
  ${allPhotos.length ? `<div class="section"><div class="section-label">Photo${allPhotos.length > 1 ? 's' : ''} (${allPhotos.length})</div><div class="photos-grid ${allPhotos.length === 1 ? 'one' : allPhotos.length === 2 ? 'two' : 'three'}">${allPhotos.map(url => `<img src="${url}" alt="Photo incident">`).join('')}</div></div>` : ''}
  <div class="footer">
...
async function changeStatut(id, statut) {
  ...
  if (!canChangeTicketStatus()) {
    toast('Vous n\'avez pas les droits pour modifier ce statut', 'err');
    return;
  }
...
async function submitComment(ticketId) {
  ...
  // Mentions (seulement pour les managers, pas le syndic)
  if (!isSyndic()) {
...
async function onCommentInput(e) {
  if (isSyndic()) return;  // Pas de mentions pour le syndic
  const ta = e.target;
  const val = ta.value;
...
function insertMention(userId, displayNameStr) {
  ...
  const newBefore = before.replace(/@\w*$/, '@' + displayNameStr + ' ');
